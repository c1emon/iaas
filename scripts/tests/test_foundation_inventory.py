...
from scripts.foundation_inventory.cli import main as foundation_main
from scripts.foundation_inventory.health import ProbeOutcome, run_health_checks
from scripts.foundation_inventory.model import build_model
from scripts.foundation_inventory.render import build_markdown
from scripts.foundation_inventory.validation import validate_foundation_inventory
from scripts.pve_inventory.checks.results import render_report


ROOT = Path(__file__).resolve().parents[2]
INVENTORY_PATH = ROOT / "inventory" / "foundation.yml"
GENERATED_DOCS_PATH = ROOT / "docs" / "generated" / "foundation-recovery.md"
...
def test_offline_check_mode_does_not_run_health(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("scripts.foundation_inventory.cli.run_health_checks", _boom)
...
def test_foundation_cli_validation_failure_exits_1_without_traceback(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "scripts.foundation_inventory.cli", "--inventory", str(inventory_copy)],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
