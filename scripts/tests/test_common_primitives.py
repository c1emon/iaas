...
from scripts.common.io import check_outputs, check_text_file, load_json, load_yaml, write_text
...
def test_services_inventory_does_not_import_pve_shared_primitives() -> None:
    forbidden = {
        "scripts.pve_inventory.errors",
        "scripts.pve_inventory.io",
        "scripts.pve_inventory.validation_common",
    }
    for path in (ROOT / "scripts" / "services_inventory").glob("*.py"):
