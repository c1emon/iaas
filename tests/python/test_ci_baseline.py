...
def test_ci_runs_native_opentofu_without_action_wrapper():
    ...
    for filename, job in (('offline-validation.yml', 'check'), ('oci-release.yml', 'build')):
...
def test_ci_uses_runtime_tools_and_explicit_collection_closure():
    ...
    runtime = yaml.safe_load((ROOT / 'automation/oci/common/collections.yml').read_text())
    ...
    assert dockerfile.index('COPY pyproject.toml uv.lock') < dockerfile.index('COPY src/')
...
def test_image_jobs_use_one_platform_capable_docker_baseline():
    ...
    for filename, names in (
        ...
        ('oci-release.yml', ('build', 'publish', 'anonymous-consumption')),
    ):
...
def test_ci_upload_uses_injected_token_without_local_shell_bootstrap():
    workflow = yaml.safe_load((ROOT / '.github/workflows/oci-release.yml').read_text())
    ...
    for filename in ('offline-validation.yml', 'oci-release.yml'):
