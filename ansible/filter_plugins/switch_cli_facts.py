"""Filters for parsing SKS8300 switch CLI output."""
...
def normalize_switch_telnet_outputs(commands: list[str], outputs: Any) -> dict[str, str]:
    """Map ansible.netcommon.telnet output to command keys.

    The module has changed output shape across versions/devices. Some runs return
    one entry per command including pagination, some return only command output,
    and some devices return one concatenated transcript. Keep parsing resilient.
    """
    if outputs is None:
        text_outputs = []
        ...
        text_outputs = ["" if value is None else str(value) for value in outputs]
    show_commands = [command for command in commands if command.lower().startswith("show ")]
    result = {_command_key(command): "" for command in show_commands}
    ...
    if len(text_outputs) == len(commands):
        for command, output in zip(commands, text_outputs, strict=False):
            if command.lower().startswith("show "):
                result[_command_key(command)] = output
        return result
    ...
    if transcript:
        for index, command in enumerate(show_commands):
            start = transcript.lower().find(command.lower())
            if start == -1:
                continue
            following_starts = [
                position
                for next_command in show_commands[index + 1 :]
                if (position := transcript.lower().find(next_command.lower(), start + len(command))) != -1
            ]
            end = min(following_starts) if following_starts else len(transcript)
            result[_command_key(command)] = transcript[start:end].strip()
...
def parse_switch_show_version(output: str) -> dict[str, Any]:
    ...
    facts = {
        ...
        "cpu_mac": _normalize_mac(
            _first_match(output, [r"\bcpu\s+mac(?:\s+address)?\s*[:=]?\s*([0-9a-f:-]{17})"])
        ),
        "vlan_mac": _normalize_mac(
            _first_match(output, [r"\bvlan\s+mac(?:\s+address)?\s*[:=]?\s*([0-9a-f:-]{17})"])
        ),
...
def switch_cli_facts(outputs: dict[str, str], inventory_hostname: str = "") -> dict[str, Any]:
    ...
    running_config = outputs.get("running_config", "")
    return {
...
class FilterModule:
    ...
    def filters(self) -> dict[str, Any]:
        return {
            "normalize_switch_telnet_outputs": normalize_switch_telnet_outputs,
