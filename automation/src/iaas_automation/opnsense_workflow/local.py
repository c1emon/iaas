...
Run from the checkout with ``PYTHONPATH=automation/src uv run python -m
iaas_automation.opnsense_workflow.local --help``.
...
def source_identity(root: Path) -> dict:
    ...
    for directory in ('automation/src', 'automation/ansible'):
...
def run_local(args: argparse.Namespace) -> dict:
    ...
    environ['PYTHONPATH'] = str(root / 'automation/src')
