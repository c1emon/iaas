...
from iaas_automation.common.validation import as_list, as_mapping, require_non_empty_string, require_positive_int, require_unknown_keys
...
def load_vm_names(vms_doc: dict[str, Any]) -> set[str]:
    """Collect declared VM names from the supplied VM inventory."""
...
def validate_services(doc: dict[str, Any], vm_names: set[str]) -> tuple[list[dict[str, Any]], list[dict[str, str]]]:
    ...
    for service_index, service in enumerate(services):
        ...
        require(owner_vm in vm_names, f"services inventory.services[{service_index}]: owner_vm {owner_vm} is not declared in the VM inventory")
