...
除非特别说明，以下命令都从 `automation/ansible/` 目录执行。
...
```text
../../environments/astra/ansible/vars/opnsense/aliases.yml
```
...
```text
../../environments/astra/ansible/vars/opnsense/vips.yml
```
...
- 这是写入型 playbook；首次运行前先执行 `snapshot.yml`。
- API 凭据只通过 `OPNSENSE_API_KEY` 和 `OPNSENSE_API_SECRET` 环境变量注入，不写入仓库。
- `../../environments/astra/ansible/vars/opnsense/vips.yml` 是手写 desired state，不由 `../../exports/opnsense/` 下的导出文件生成。
...
```text
../../environments/astra/ansible/vars/opnsense/gateways.yml
```
...
- 这是写入型 playbook；首次运行前先执行 `snapshot.yml`。
- API 凭据只通过 `OPNSENSE_API_KEY` 和 `OPNSENSE_API_SECRET` 环境变量注入，不写入仓库。
- `../../environments/astra/ansible/vars/opnsense/gateways.yml` 是手写 desired state，不由 `../../exports/opnsense/` 下的导出文件生成。
...
`automation/ansible/requirements.yml` 精确锁定 `oxlorg.opnsense` 26.1.11。该版本将 API 有时缺失的
...
```text
../../environments/astra/ansible/vars/opnsense/filter-rules.yml
```
...
- 这是写入型 playbook；首次运行前先执行 `snapshot.yml`。
- API 凭据只通过 `OPNSENSE_API_KEY` 和 `OPNSENSE_API_SECRET` 环境变量注入，不写入仓库。
- `../../environments/astra/ansible/vars/opnsense/filter-rules.yml` 是手写 desired state，不由 `download_rules.csv` 或
...
```text
../../environments/astra/ansible/vars/opnsense/dnat.yml
```
...
预期结果：playbook 在读取 `../../environments/astra/ansible/vars/opnsense/dnat.yml` 后失败，并输出 DNAT 管理未实现的提示。
