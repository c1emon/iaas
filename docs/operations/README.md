...
本目录是本仓库通用基础设施自动化的**人工操作手册**。调用方选择环境并准备配置，按需执行从
...
它是执行方式说明，不替代下列基础设施准入与验收步骤。
所有路径变量在第 0 章显式设置；本文不预设环境名称、域名、主机或存储 ID。
...
| 标记 | 含义 |
| --- | --- |
| **已实现** | 仓库中有可调用的代码、playbook 或 Make 入口；是否已在目标环境运行仍需看该次运行证据。 |
...
基础，调用方须自行提供 K3s 节点组合和 intent。某环境的实录不代表其他环境已部署。Cilium、Flux、
...
| 调用方环境声明 | `$ENVIRONMENT_DIR/inventory/`、`$ENVIRONMENT_DIR/ansible/` | 指出每个字段的含义、编辑顺序和验证方式。 |
| 运行时凭据 | 调用方环境变量、引用模板与仓库外受保护文件 | 说明注入方式和文件权限；绝不记录明文。 |
| 生成物 | `$GENERATED_DIR/` | 说明如何生成、审查和检查新鲜度；不得手工编辑。 |
