...
The system SHALL provide one supported PVE inventory command surface under `iaas.pve_inventory` after the hard cutover.
...
- **THEN** it SHALL use the documented `iaas.pve_inventory.*` module entrypoint
...
The system SHALL organize PVE cloud-init helper internals into responsibility-oriented modules under `iaas.pve_inventory` while preserving the canonical cloud-init command behavior.

#### Scenario: Cloud-init command facade remains stable
- **WHEN** an operator or root Makefile target invokes cloud-init behavior through `iaas.pve_inventory.cloud_init`
...
The system SHALL organize offline PVE inventory model, rendering, and validation helpers under `iaas.pve_inventory.inventory` rather than a flat or compatibility package.
...
- **THEN** model assembly, rendering, cluster validation, VM validation, and passthrough validation SHALL be grouped under `iaas.pve_inventory.inventory`
- **AND** the canonical `iaas.pve_inventory.cli` entrypoint SHALL receive environment source and output paths explicitly
...
- **THEN** transport, runtime, error, and protocol concerns SHALL remain under `iaas.pve_inventory.pve_api`
...
- **THEN** cluster validation SHALL be imported from `iaas.pve_inventory.inventory.validation.cluster`
- **AND** VM validation SHALL be imported from `iaas.pve_inventory.inventory.validation.vm`
...
The system SHALL organize PVE health current-state helpers under `iaas.pve_inventory.checks.health` while preserving the canonical health command.
...
- **THEN** expectation derivation and current-state checks SHALL be grouped under `iaas.pve_inventory.checks.health`
- **AND** `iaas.pve_inventory.health` SHALL be the sole command-facing module entrypoint
...
- **AND** transport/runtime/error concerns SHALL remain in `iaas.pve_inventory.pve_api`
