...
- **THEN** those shared helpers SHALL be available from `iaas.common`
- **AND** callers SHALL NOT need to import domain-neutral helpers through `iaas.pve_inventory` or `iaas.services_inventory`
...
- **THEN** it SHALL import that helper behavior from `iaas.common`
...
- **THEN** `iaas.common` SHALL NOT import from `iaas.pve_inventory` or `iaas.services_inventory`
