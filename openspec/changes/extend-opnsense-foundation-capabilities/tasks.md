...
- [x] 0.4 Implement the explicit standalone preflight mode and operation-selected deploy/upgrade modes using the repair-plan state table and shared read-only facts; reject foreign/ambiguous installation state without rejecting an expected installed cluster (R05).
- [x] 0.5 Stage and checksum upgrade artifacts before stopping services; preserve serial server-before-agent ordering, safe bounded restart verification and explicit failure recovery without datastore rollback (R06).
- [x] 0.6 Parse real capability bits and exact IP/interface facts; refresh and compare observed versions before upgrade, and verify exact node identity/version/readiness after each upgraded node and at completion (R07).
