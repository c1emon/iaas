...
- [x] 7.1 修复真实返回中的空 statetimeout 和 DNAT 嵌套字段适配，补充脱敏代表样例，保留来源冲突与未知字段处理。
- [x] 7.2 用同一批原生数据对照重构前 reader，确认支持对象的标准配置无损；核实保留的 manual_required 符合规格。
- [x] 7.3 运行修复后的离线检查及七类资源实际只读复测，记录设备数据覆盖、剩余边界和阶段提交；不执行设备写入。
