...
- 由调用方集中选择镜像版本和平台；检查配置格式、启动器接口与镜像兼容性。支持 Linux amd64 和 Apple Silicon 上显式 amd64 模拟，评估原生 arm64 的可支持操作。
...
- `oci-runtime-delivery`: 发布启动器及兼容元数据，明确平台能力和分组验收，保持运行镜像内容边界。
