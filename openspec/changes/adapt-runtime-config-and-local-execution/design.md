...
Linux amd64 为完整基线；Mac arm64 宿主安装原生启动器，设施容器显式选 linux/amd64 模拟，engine 必须具备对应能力。原生 linux/arm64 按已评估工具及 provider 能力记录操作支持表；缺能力的组合明确拒绝，并由调用方显式选择 amd64，不能静默模拟。真实平台未验证则标为未验证，不据此声称 native arm64 已交付。
