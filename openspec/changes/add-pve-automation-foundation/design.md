...
If future requirements need richer cloud-init content, snippets should use the isolated `images` NFS storage because it supports the `Snippets` content type and is visible cluster-wide. For Section 4A, the runtime helper should render snippets locally, upload them to `images` snippets storage via the audited host-side wrapper, reference them with `user_data_file_id`, and keep them for the VM lifetime instead of deleting them immediately. Because this storage is NFS-backed, the wrapper installs snippets with `0644` instead of `0600` so PVE can read them when root-squash or server-side ownership mapping is in effect; this is acceptable only because the NFS storage is treated as isolated infrastructure storage.
