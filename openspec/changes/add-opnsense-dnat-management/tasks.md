...
- [ ] 2.1 建立两类 NAT 公共身份和生命周期辅助，三个资源独立 validator/分派，不建立万能 NAT 字段模型；以 DNAT/1:1 NAT/Groups 的最小合法/非法输入、NAT 间同名身份合法及类内冲突拒绝、未知字段与严格类型测试为依据。DNAT 对 present 精确匹配对象增加首个写入前只读模式核对，代表性测试 `nordr` 为真或无法判定时整批拒绝、正常规则可更新及 absent 仍可精确删除，不转换免转发例外模式。
